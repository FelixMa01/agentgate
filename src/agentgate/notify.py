...
def notify_ask(
    token: str, tool: str, event: dict, rule_name: str | None, reason: str | None
) -> str:
    ...
    approval_host = os.environ.get(
        "AGENTGATE_APPROVAL_HOST", f"127.0.0.1:{os.environ.get('AGENTGATE_APPROVAL_PORT', '8765')}"
    )
    ...
    if webhook:
        payload = build_ask_message(token, tool, event, rule_name, reason, approval_host, scheme)
    ...
    fallback = Path(
        os.environ.get("AGENTGATE_ASK_FALLBACK")
